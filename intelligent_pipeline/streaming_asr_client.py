...
class AsyncStreamingASRClient:
    """Async gRPC Client for Streaming ASR Triton Server."""
    ...
    async def connect(self):
        """Connect to Triton streaming ASR server."""
        self.client = grpcclient_aio.InferenceServerClient(url=self.url)

        if not await self.client.is_server_live():
            raise Exception(f"Streaming ASR server at {self.url} is not live")
        if not await self.client.is_server_ready():
            raise Exception(f"Streaming ASR server at {self.url} is not ready")
    ...
    async def transcribe_chunk(
        ...
        return self._decode_json_output(response.as_numpy("transcription"))
